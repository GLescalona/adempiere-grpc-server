...
public class InvoiceFiscalDocumentLine implements IFiscalDocumentLine {
    ...
    private void convertDocument(MInvoiceLine invoiceLine) {
        ...
        if (withholding != null && withholding.get_ID() > 0) {
            ...
            this.withholdingBaseAmount = withholding.getA_Base_Amount();
            //	Related (source) document: the original invoice being withheld/perceived
            if (withholding.getSourceInvoice_ID() > 0) {
                MInvoice sourceInvoice = new MInvoice(invoiceLine.getCtx(), withholding.getSourceInvoice_ID(), invoiceLine.get_TrxName());
                this.relatedDocument = new RelatedDocument(sourceInvoice).withBaseAmount(withholding.getA_Base_Amount());
            }
        }
